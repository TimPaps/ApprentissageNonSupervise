...
\end{Verbatim}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}6}]:} \PY{c+c1}{\PYZsh{} Import des données csv}
        ...
        \PY{n}{df0}\PY{o}{.}\PY{n}{loc}\PY{p}{[}\PY{p}{:}\PY{p}{,}\PY{l+m+mi}{0}\PY{p}{:}\PY{l+m+mi}{4}\PY{p}{]}\PY{o}{.}\PY{n}{head}\PY{p}{(}\PY{n}{n}\PY{o}{=}\PY{l+m+mi}{5}\PY{p}{)}
\end{Verbatim}
...
\end{Verbatim}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}7}]:} \PY{c+c1}{\PYZsh{} Transformation des variables catégorielles (produits) en variables binaires pour chaque produits (achat/pas achat)}
...
\begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}8}]:} PCA(copy=True, iterated\_power='auto', n\_components=None, random\_state=None,
...
composantes.
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}32}]:} \PY{n}{plt}\PY{o}{.}\PY{n}{figure}\PY{p}{(}\PY{n}{figsize}\PY{o}{=}\PY{p}{(}\PY{l+m+mi}{5}\PY{p}{,}\PY{l+m+mi}{5}\PY{p}{)}\PY{p}{)}
...
\end{Verbatim}
    ...
    \begin{center}
    \adjustimage{max size={0.5\linewidth}{0.5\paperheight}}{output_11_0.png}
    \end{center}
    { \hspace*{\fill} \\}
...
80 premières n'expliquent que 90\%.
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}33}]:} \PY{n}{PCs} \PY{o}{=} \PY{n}{pca}\PY{o}{.}\PY{n}{components\PYZus{}}
...
\begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}33}]:} Text(0.5,1,'Représentation de chaque produit sur les 2 premières composantes')
\end{Verbatim}
            
    \begin{center}
    \adjustimage{max size={0.5\linewidth}{0.5\paperheight}}{output_13_1.png}
    \end{center}
    { \hspace*{\fill} \\}
...
\begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}34}]:} <mpl\_toolkits.mplot3d.art3d.Path3DCollection at 0x2395c505668>
\end{Verbatim}
            
    \begin{center}
    \adjustimage{max size={0.5\linewidth}{0.5\paperheight}}{output_17_1.png}
    \end{center}
    { \hspace*{\fill} \\}
...
achetés}\label{top-10-des-produits-les-plus-achetuxe9s}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}13}]:} \PY{n}{df}\PY{o}{.}\PY{n}{sum}\PY{p}{(}\PY{p}{)}\PY{o}{.}\PY{n}{sort\PYZus{}values}\PY{p}{(}\PY{n}{ascending}\PY{o}{=}\PY{k+kc}{False}\PY{p}{)}\PY{o}{.}\PY{n}{nlargest}\PY{p}{(}\PY{l+m+mi}{10}\PY{p}{)}
\end{Verbatim}


\begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}13}]:} whole milk          2513
...
ayant essentiellement du lait - cluster 2: paniers ayant essentiellement
des sodas - cluster 3: paniers ayant du lait, légupes et fruits -
cluster 4: paniers ayant des rolls, bieres, eaux et des sac (surement
pour les bouteilles) - cluster 5: paniers ayant des yaourts, lait et
légumes

On peut conclure que les sodas, les bieres et les légumes/fruits sont
achetés séparément. Ceci est cohérent au vue des résultats eu avec la
PCA.

    \subsubsection{Agglomerative Clustering}\label{agglomerative-clustering}

    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}20}]:} \PY{k+kn}{import} \PY{n+nn}{scipy}\PY{n+nn}{.}\PY{n+nn}{cluster}\PY{n+nn}{.}\PY{n+nn}{hierarchy} \PY{k}{as} \PY{n+nn}{sch}
         \PY{k+kn}{from} \PY{n+nn}{sklearn}\PY{n+nn}{.}\PY{n+nn}{cluster} \PY{k}{import} \PY{n}{AgglomerativeClustering}
\end{Verbatim}


    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}35}]:} \PY{n}{x} \PY{o}{=} \PY{n}{df}\PY{o}{.}\PY{n}{values} \PY{c+c1}{\PYZsh{}x\PYZus{}pca}
         \PY{n}{plt}\PY{o}{.}\PY{n}{figure}\PY{p}{(}\PY{n}{figsize}\PY{o}{=}\PY{p}{(}\PY{l+m+mi}{5}\PY{p}{,} \PY{l+m+mi}{5}\PY{p}{)}\PY{p}{)}
...
à 5.
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}25}]:} \PY{c+c1}{\PYZsh{}n\PYZus{}clusters = 5}
...
\end{Verbatim}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}26}]:} \PY{k}{for} \PY{n}{i} \PY{o+ow}{in} \PY{n+nb}{range} \PY{p}{(}\PY{l+m+mi}{1}\PY{p}{,}\PY{n}{n\PYZus{}clusters}\PY{o}{+}\PY{l+m+mi}{1}\PY{p}{)}\PY{p}{:}
             \PY{n+nb}{print}\PY{p}{(}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+se}{\PYZbs{}n}\PY{l+s+s1}{ Top 10 des produits les plus achetés dans le cluster }\PY{l+s+s1}{\PYZsq{}} \PY{o}{+} \PY{n+nb}{str}\PY{p}{(}\PY{n}{i}\PY{p}{)}\PY{p}{)}
...
AC}\label{comparaison-k-means-et-ac}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}27}]:} \PY{k+kn}{from} \PY{n+nn}{sklearn}\PY{n+nn}{.}\PY{n+nn}{metrics} \PY{k}{import} \PY{n}{confusion\PYZus{}matrix}
         \PY{n}{mat} \PY{o}{=} \PY{n}{confusion\PYZus{}matrix}\PY{p}{(}\PY{n}{labels\PYZus{}ac}\PY{p}{,}\PY{n}{labels\PYZus{}km}\PY{p}{)}
...
\begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}27}]:} Text(0,0.5,'Agglomerative clustering')
\end{Verbatim}
...
2 méthodes: - cluster KM 1 \textless{}=\textgreater{} cluster AC 3
(cluster des lait, yaourt, légumes et fruit) - cluster KM 2
\textless{}=\textgreater{} cluster AC 1 (cluster des boissons) - cluster
KM 3 \textless{}=\textgreater{} cluster AC 2 (cluster légumes et lait) -
cluster KM 4 \textless{}=\textgreater{} cluster AC 1 (cluster boissons)
- cluster KM 5 \textless{}=\textgreater{} cluster AC 3 (cluster des
lait, yaourt, légumes et fruit)
...
d'association.
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}36}]:} \PY{k+kn}{from} \PY{n+nn}{mlxtend}\PY{n+nn}{.}\PY{n+nn}{frequent\PYZus{}patterns} \PY{k}{import} \PY{n}{apriori}
         ...
         \PY{n}{frequent\PYZus{}itemsets}\PY{p}{[}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{length}\PY{l+s+s1}{\PYZsq{}}\PY{p}{]} \PY{o}{=} \PY{n}{frequent\PYZus{}itemsets}\PY{p}{[}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{itemsets}\PY{l+s+s1}{\PYZsq{}}\PY{p}{]}\PY{o}{.}\PY{n}{apply}\PY{p}{(}\PY{k}{lambda} \PY{n}{x}\PY{p}{:} \PY{n+nb}{len}\PY{p}{(}\PY{n}{x}\PY{p}{)}\PY{p}{)}
         \PY{n}{frequent\PYZus{}itemsets}\PY{p}{[} \PY{p}{(}\PY{n}{frequent\PYZus{}itemsets}\PY{p}{[}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{length}\PY{l+s+s1}{\PYZsq{}}\PY{p}{]} \PY{o}{==} \PY{l+m+mi}{2}\PY{p}{)}\PY{p}{]}
\end{Verbatim}


\begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}36}]:}      support                        itemsets  length
...
lait et légumes - lait et rolls/buns - lait et yaourt
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}30}]:} \PY{k+kn}{from} \PY{n+nn}{mlxtend}\PY{n+nn}{.}\PY{n+nn}{frequent\PYZus{}patterns} \PY{k}{import} \PY{n}{association\PYZus{}rules}
...
\begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}30}]:}           antecedents         consequents  antecedent support  \textbackslash{}
